...
//	std::vector<string> spl = StringHelper::split(scene.get_title(), ':');
//	string filename = StringHelper::sanitize_filename(StringHelper::join(spl, ""));
	...
	// lights
	vec4 ambient_color = lights.ambient;
	fprintf(out, "\n// LIGHTS\n\n");
	fprintf(out, "# declare lum = 6;\n");
	fprintf(out, "global_settings {\n\
	ambient_light rgb lum * <%g, %g, %g>\n\
	max_trace_level 15\n\
}", ambient_color[0], ambient_color[1], ambient_color[2]);

	fprintf(out, "# declare cpy_direct_light_amount = %g;\n", lights.get_light(0).diffuse[0]);
	for(int i = 0; i < 8; i++) {
		Light l = lights.get_light(i);
		if(l.enabled) {
			vec4 pos = view * l.position;
			fprintf(out, "light_source\n{");
			// we put 1000 here to mimic directional light sources since povray has only point light sources
			fprintf(out, "\t1000 * <%g, %g, %g>,\n", pos[0], pos[1], pos[2]);
			fprintf(out, "\trgb lum * cpy_direct_light_amount\n");
