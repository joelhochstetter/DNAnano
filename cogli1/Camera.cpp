...
void Camera::load_state_from_file(std::ifstream &in) {
	...
	while(in.good()) {
		...
		if(spl.size() > 0 && buff[0] != '#') {
			if(spl[0] == "fov") StringHelper::from_string<float>(spl[1], _fov);
			else if(spl[0] == "position") StringHelper::get_vec3(spl, 1, _position);
			else if(spl[0] == "right") StringHelper::get_vec3(spl, 1, _right);
			else if(spl[0] == "up") StringHelper::get_vec3(spl, 1, _up);
			else if(spl[0] == "look_at") StringHelper::get_vec3(spl, 1, _look_at);
			else if(spl[0] == "ortho") StringHelper::from_string<bool>(spl[1], _is_ortho);
...
void Camera::_set_projection_matrix() {
	if(!_is_ortho) _projection_matrix = glm::perspective(_fov, window_ratio(), 0.01f, 10000.0f);
	else {
		...
		_projection_matrix = glm::ortho(-half_width, half_width, -half_height, half_height, 0.01f, 1000.0f);
	}
}
...
void Camera::update(InputState &input, float dt) {
	...
	if(input.right_button) {
		if(!_right_button) _right_button = true;
		else {
			delta_horizontal_angle = _mouse_speed * (input.cursor_pos_x - _ref_cursor_pos_x);
			delta_vertical_angle = _mouse_speed * (input.cursor_pos_y - _ref_cursor_pos_y);
		}
		_ref_cursor_pos_x = input.cursor_pos_x;
		_ref_cursor_pos_y = input.cursor_pos_y;
	}
	else _right_button = false;

	if(input.left_button) {
		if(!_left_button) _left_button = true;
		else {
			delta_horizontal_angle = -_mouse_speed * (input.cursor_pos_x - _ref_cursor_pos_x);
			delta_vertical_angle = -_mouse_speed * (input.cursor_pos_y - _ref_cursor_pos_y);
		}
	...
	vec3 dpos(0.f, 0.f, 0.f);

	if(input.left) dpos -= _right * _kb_speed * dt;
	if(input.right) dpos += _right * _kb_speed * dt;
	if(input.up) dpos += _up * _kb_speed * dt;
	if(input.down) dpos -= _up * _kb_speed * dt;
	if(input.forward) {
		dpos += direction * 10.f * _kb_speed * dt;
		input.forward = false;
	}
	if(input.backward) {
		dpos -= direction * 10.f * _kb_speed * dt;
	...
	if(input.fast_forward) {
		dpos += direction * 50.f * _kb_speed * dt;
	...
	if(_left_button || fix_position) _position = _look_at - distance*direction;
	else _look_at = _position + distance*direction;
	...
	if(input.fov_change != 0.f) {
		...
		if(_fov <= 4.f) _fov = 4.f;
		if(_fov >= 180.f) _fov = 179.f;
	...
	if(input.window_height != -1) _window_height = input.window_height;
	if(input.window_width != -1) _window_width = input.window_width;
