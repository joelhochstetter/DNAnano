...
Scene::~Scene() {
	for(vector<Shape *>::iterator it = _shapes.begin(); it != _shapes.end(); it++)
		delete *it;
	for(vector<Cylinder *>::iterator it = _box.begin(); it != _box.end(); it++)
		delete *it;
	for(vector<BaseAction *>::iterator it = _actions.begin(); it != _actions.end(); it++)
		delete *it;
}
...
void Scene::set_actions(std::vector<BaseAction *> actions) {
	_actions = actions;
	_current_action = 0;
}
...
std::string Scene::get_title() {
	...
	if(_current_action > 0) title += StringHelper::sformat(", action %u", _current_action);
	return title;
}
...
bool Scene::previous_action() {
	...
	if(_current_action == 0) return false;
...
bool Scene::next_action() {
	...
	if(_current_action == _actions.size()) return false;
...
void Scene::hide_selected() {
	for(vector<Shape *>::iterator it = _shapes.begin(); it != _shapes.end(); it++)
		if((*it)->is_selected()) (*it)->set_visible(false);
}
...
void Scene::select_shape(int id) {
	if(id < 0 || id > (int) _shapes.size()) Logging::log_warning("The shape n. %d does not exist: it cannot be select\n", id);
	else _shapes[id]->toggle_selected();
}
...
void Scene::set_box(glm::vec3 &b) {
	...
	_box.push_back(new Cylinder(vec3(0.f, 0.f, 0.f), vec3(b[0], 0.f, 0.f), r, bc));
	_box.push_back(new Cylinder(vec3(0.f, 0.f, 0.f), vec3(0.f, b[1], 0.f), r, bc));
	_box.push_back(new Cylinder(vec3(0.f, 0.f, 0.f), vec3(0.f, 0.f, b[2]), r, bc));

	_box.push_back(new Cylinder(b, vec3(-b[0], 0.f, 0.f), r, bc));
	_box.push_back(new Cylinder(b, vec3(0.f, -b[1], 0.f), r, bc));
	_box.push_back(new Cylinder(b, vec3(0.f, 0.f, -b[2]), r, bc));

	_box.push_back(new Cylinder(vec3(0.f, 0.f, b[2]), vec3(0.f, b[1], 0.f), r, bc));
	_box.push_back(new Cylinder(vec3(0.f, 0.f, b[2]), vec3(b[0], 0.f, 0.f), r, bc));

	_box.push_back(new Cylinder(vec3(b[0], b[1], 0.f), vec3(-b[0], 0.f, 0.f), r, bc));
	_box.push_back(new Cylinder(vec3(b[0], b[1], 0.f), vec3(0.f, -b[1], 0.f), r, bc));

	_box.push_back(new Cylinder(vec3(b[0], 0.f, b[2]), vec3(0.f, 0.f, -b[2]), r, bc));

	_box.push_back(new Cylinder(vec3(0.f, b[1], 0.f), vec3(0.f, 0.f, b[2]), r, bc));
...
void Scene::translate(vec3 amount, bool is_global) {
	if(is_global) Scene::global_shift += amount;
	_shift += amount;
...
void Scene::centre(CentreOptions &centre) {
	...
	translate(my_shift);
	if(centre.consider_inertia) align_according_to_inertia_tensor(new_com);
}
...
void Scene::align_according_to_inertia_tensor(vec3 wrt) {
	...
	for(vector<Shape *>::iterator it = _shapes.begin(); it != _shapes.end(); it++) {
		Shape *s = *it;
		vec3 shift = R * (s->position() - wrt) - s->position() + wrt;
		s->shift(shift);
		s->rotate(R, s->position());
	}
}
...
void Scene::load_state_from_file(std::ifstream &in) {
	...
	while(in.good()) {
		...
		if(spl.size() > 0 && buff[0] != '#') {
			if(spl[0] == "visible_box") StringHelper::from_string<bool>(spl[1], Scene::_show_box);
			else if(spl[0] == "shift") StringHelper::get_vec3(spl, 1, Scene::global_shift);
		}
	}
}
