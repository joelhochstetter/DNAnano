...
DNAParser::DNAParser(string topology, const std::queue<string> &input_files) : Parser(input_files) {
	...
	try {
		...
		while(!done) {
			...
			if(buff[0] == 't' && spl.size() == 3) {
				...
				nucleotides.push_back(new Nucleotide(pos, v1, v3, color));
			}
			curr_line++;
		...
		if (_options[COLORS_FROM]) {
			...
			while (colors_file.good()) {
				...
				int id = atoi(spl[0].c_str());
				string colorstring = "C[" + spl[1] + "]";
				vec4 color = _get_color(colorstring);

				if (id < 0 || id >= (int)nucleotides.size()) Logging::log_info("Ignoring invalid nucleotide id %d in colors_from file\n", id);
				else nucleotides[id]->set_color(color);
...
void DNAParser::_read_topology (std::string filename) {
		...
		for (int i = 0; i < _N_nuc; i ++) {
			...
			_n2s_map[i] = k;
			if(k > (int)_strand_circularity.size()) _strand_circularity.resize(k, true);
			if(p == -1 || q == -1) _strand_circularity[k-1] = false;
		}
