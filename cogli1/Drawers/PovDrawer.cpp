...
PovDrawer::PovDrawer() :
		_out(NULL), _dna_as_ribbon(false), _patchy_as_kf(false), _patch_header_printed(false), _draw_outlines(false), _draw_shadows(false) {
...
void PovDrawer::visit(Scene &s) {
	...
	for(vector<Shape *>::iterator it = objs.begin(); it != objs.end(); it++) {
		Shape *my_s = *it;
		if(my_s->is_visible()) my_s->accept_drawer(*this);
	}

	if(s.is_box_shown()) {
		fprintf(_out, "// box output\n");
		vector<Cylinder *> &box = s.get_box();
		for(vector<Cylinder *>::iterator it = box.begin(); it != box.end(); it++)
			(*it)->accept_drawer(*this);
...
void PovDrawer::visit(Cylinder &c) {
	...
	if(!_draw_shadows) fprintf(_out, "\tno_shadow\n}\n");
