...
{UNKNOWN               , 0, "" , ""              , option::Arg::None,     "USAGE: cogli1 [options] input_1 input_2 ...\n\nOptions:" },
...
#ifdef OXDNA
{WITH_OXDNA     , 0, " ", "oxDNA"         , option::Arg::None,     "  --oxDNA\n          enable oxDNA mode"},
{GRANULARITY    , 0, "g", "granularity"   , option::Arg::Required, "  --granularity, -g value\n          set the number of steps to be updated when a configuration is changed in oxDNA mode"},
#endif
{0,0,0,0,0,0}
};
...
int main(int argc, char *argv[]) {
	...
	for(unsigned int i = 0; i < stats.options_max; i++) v_options.push_back(options[i]);
	Cogli1 c1(parser, options);

	if(options[ONLY_POV]) c1.print_all_pov();
	else c1.start_ogl();
