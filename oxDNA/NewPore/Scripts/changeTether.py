...
jsonfile  = sys.argv[1]
teth_len  = int(sys.argv[3])
t3p_p     = int(sys.argv[3])
t3p_b     = int(sys.argv[4])
t5p_p     = int(sys.argv[5])
t5p_b     = int(sys.argv[7])
...
for i in range(len(vstrands[t3p_p]["scaf"])):
    ...
    if vstrands[t3p_p]["scaf"][i][0] != -1 and vstrands[t3p_p]["scaf"][i][0] != vstrands[t3p_p]["num"]:
        bp_3p_p = vstrands[t3p_p]["scaf"][i][0]
        vs_3p_p = vstrands[t3p_p]["scaf"][i][1]
        break
...
for i in range(len(vstrands[t5p_p]["scaf"])):
    ...
    if vstrands[t5p_p]["scaf"][i][2] != -1 and vstrands[t5p_p]["scaf"][i][2] != vstrands[t5p_p]["num"]:
        bp_5p_p = vstrands[t5p_p]["scaf"][i][2]
        vs_5p_p = vstrands[t5p_p]["scaf"][i][3]
        break
...
fnameLs = split(jsonfile, '.')
writefile = fnameLs(0) + '_tether_n_' + str(n) + '.json'
