...
def getBonds():
    ...
    while mysystem != False:
        ...
        if counter == confid:
            ...
            #print >> sys.stderr, mystderr #cancel this out
    ...
    print(numBonds)
    np.savetxt(savefile, np.array(numBonds), fmt='%i') 
    print savefile


def getNumberOfBonds(bondfile):
    track = 0
    numBonds = 0
    with open(bondfile, 'r') as r:
        for line in r:
            cells = line.split()
            if line[0] == '#':
                track += 1
